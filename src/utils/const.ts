// const
const MAPTILER_KEY = 'odrNKyXlNhi0YbvBPcMm';
const MAP_STYLE = `https://api.maptiler.com/maps/ch-swisstopo-lbm-dark/style.json?key=${MAPTILER_KEY}`;
...
const CHINESE_INFO_MESSAGE = (yearLength: number, year: string): string => {
  ...
  return `选择了长跑就是选择了长期主义。每一步都算数：持续记录自己跑步 ${yearLength} 年了，下面列表展示的是${yearStr}的数据`;
};
...
const nike = '#FFA630'; // if you want change the main color change here src/styles/variables.scss
